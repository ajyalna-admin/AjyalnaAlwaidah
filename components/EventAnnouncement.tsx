...
export function EventAnnouncement({ storageKey, delayMs = 500 }: EventAnnouncementProps) {
  const [visible, setVisible] = useState(false);
  const [mounted, setMounted] = useState(false);
  const pathname = usePathname();

  useEffect(() => {
    // لا تظهر داخل صفحة تفاصيل الجلسة نفسها.
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={transitionSession.title}
    >
      <button
        onClick={close}
        aria-label="إغلاق"
